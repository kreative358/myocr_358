...
class UnitTest:
    def __init__(self, 
        ...
        easy_ocr_init = os.path.join(myocr_358_module, "__init__.py")
        if not os.path.isfile(easy_ocr_init):
            raise FileNotFoundError("Invalid myocr_358_module. The directory should contain __init__.py.")
        
        spec = importlib.util.spec_from_file_location("myocr_358", easy_ocr_init)
        myocr_358 = importlib.util.module_from_spec(spec)
        sys.modules["myocr_358"] = myocr_358
        spec.loader.exec_module(myocr_358)
        
        self.myocr_358 = myocr_358
        if not hasattr(self.myocr_358, 'utils'):
            setattr(self.myocr_358, 'utils', importlib.import_module('myocr_358.utils'))
        if not hasattr(self.myocr_358, 'detection'):
            setattr(self.myocr_358, 'detection', importlib.import_module('myocr_358.detection'))
        if not hasattr(self.myocr_358, 'recognition'):
            setattr(self.myocr_358, 'recognition', importlib.import_module('myocr_358.recognition'))
        
        self.myocr_358_dir = os.path.dirname(myocr_358.__file__)
        
        print("Unit test is set for myocr_358 at {}".format(os.path.abspath(self.myocr_358_dir)))
    ...
    def set_data(self, test_data):
        ...
        english_mini_bgr, english_mini_gray = self.myocr_358.utils.reformat_input(images['english']['mini'])
        english_small_bgr, english_small_gray = self.myocr_358.utils.reformat_input(images['english']['small'])
        ...
        setattr(self.inputs, 'myocr_358_config', self.dict2attr(solution_book['inputs']['myocr_358_config']))
    ...
    def myocr_358_read_as(self, image, language):
        if not isinstance(language, list):
            language = [language]
        reader =  self.myocr_358.Reader(language)
        _, pred, confidence = reader.readtext(image)[0]
        reader = None
        torch.cuda.empty_cache()
        return pred, confidence
    
    def set_myocr_358(self):
        ocr = self.myocr_358.Reader([self.inputs.myocr_358_config.main_language])
        setattr(self.myocr_358, 'ocr', ocr)
    ...
    def do_test(self, verbose = None):
        ...
        num_module_pass = 0
        print("Testing myocr_358: {:d} modules will be tested.\n".format(num_module_to_test))
