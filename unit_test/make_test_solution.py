...
def get_myocr_358(language):
    if not isinstance(language, list):
        language = [language]
    return myocr_358.Reader(language)

# %%
def main(args):
    
    if args.output is None:
        args.output = "myocr_358UnitTestPackage_{}.pickle".format(datetime.now().strftime("%Y%m%dT%H%M"))
    ...
    myocr_358_config = {"main_language": 'en'}
    
    ocr = get_myocr_358(myocr_358_config["main_language"])
    ...
    english_mini_bgr, english_mini_gray = myocr_358.utils.reformat_input(images['english']['mini'])
    english_small_bgr, english_small_gray = myocr_358.utils.reformat_input(images['english']['small'])
    
    
    model_init_test = {'test01': {
                            'description': "Counting parameters of detector module.",
                            "method": "unit_test.count_parameters",
                            'input': ["unit_test.myocr_358.ocr.detector"],
                      ...
                      'test03': {
                            ...
                            "method": "unit_test.count_parameters",
                            'input': ["unit_test.myocr_358.ocr.recognizer"],
    ...
    get_textbox_test.update({'test01': {
                                ...
                                "method": "unit_test.myocr_358.detection.get_textbox",
                                'input': replace(input0, 
                                                 [0, 1], 
                                                 ["unit_test.myocr_358.ocr.detector",
                                                  "unit_test.inputs.images.english.mini_bgr"
                                                   ]),
                                'output': myocr_358.detection.get_textbox(*input0),    
                                'severity': "Error"
                                }})
    ...
    output0 = myocr_358.detection.get_textbox(*input0)
    polys = output0[0]
    group_text_box_test = {}
    ...
    group_text_box_test.update({'test01': {
                                'description': "Testing with default input.",
                                "method": "unit_test.myocr_358.utils.group_text_box",
                                'input': input_,
                                'output': myocr_358.utils.group_text_box(*input_),    
                                'severity': "Error"
                                }
                            })
    input_ = [polys, 
    ...
    detect_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_358.ocr.detect",
    ...
    get_image_list_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_358.utils.get_image_list",
                        'input': input_,
                        'output': myocr_358.utils.get_image_list(*input0),    
                        'severity': "Error"
                        },
                    })
    ...
    image_list, max_width = myocr_358.utils.get_image_list(*input0)
    ...
    output_ = myocr_358.recognition.get_text(*input0)   
    input_ = replace(input0, 
                     [0, 3, 4], 
                     ["unit_test.myocr_358.ocr.character", 
                      "unit_test.myocr_358.ocr.recognizer", 
                      "unit_test.myocr_358.ocr.converter"]
                     )

    get_text_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_358.recognition.get_text",
                        'input': input_,
                        'output': output_,    
                        'severity': "Error"
                        },
    ...
    output0 = myocr_358.recognition.get_text(*input0)   
    input_ = [output0, 
              1, #x_ths
              0.5, #y_ths 
              'ltr' #mode
    ...
    get_paragraph_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_358.utils.get_paragraph",
                        'input': input_,
                        'output': myocr_358.utils.get_paragraph(*input_),    
                        'severity': "Error"
                        }})
    input_ = [output0, 
              0.5, #x_ths
    ...
    recognize_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_358.ocr.recognize",
    ...
    readtext_test = {}
    #english_tiny_bgr, _ = myocr_358.utils.reformat_input(images['english']['tiny'])
    input_ = ["unit_test.inputs.images.english.tiny", 'en']
    ocr = get_myocr_358('en')
    ...
    readtext_test.update({'test01': {
                        ...
                        "method": "unit_test.myocr_358_read_as",
                        'input': input_,
                        'output': output_,    
                        'severity': "Error"
                        }})
    #french_tiny_bgr, _ = myocr_358.utils.reformat_input(images['french']['tiny'])
    input_ = ["unit_test.inputs.images.french.tiny", 'fr']
    ocr = get_myocr_358('fr')
    ...
    #chinese_tiny_bgr, _ = myocr_358.utils.reformat_input(images['chinese']['tiny'])
    input_ = ["unit_test.inputs.images.chinese.tiny", 'ch_sim']
    ocr = get_myocr_358('ch_sim')
    ...
    #korean_tiny_bgr, _ = myocr_358.utils.reformat_input(images['korean']['tiny'])
    input_ = ["unit_test.inputs.images.korean.tiny", 'ko']
    ocr = get_myocr_358('ko')
    ...
    solution_book = {
            'inputs':{'images': image_preprocess,
                      'myocr_358_config': myocr_358_config
                      },
            'tests':{
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to pack myocr_358 weight.",
